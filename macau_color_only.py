...
#
# hk_color_two_v50.py
# 香港六合彩 · 特二色 V50 修正版（生肖映射以鼠为1，全自动权重寻优）
...
CONFIG = {
    ...
    "beast_trans_weight": 2.0,          # 特码野兽/家禽转移
    "normal_beast_main_weight": 2.5,    # 正码主导野兽/家禽转移
...
# ================== 生肖与野兽/家禽分类（香港以鼠为1） ==================
# 香港六合彩生肖顺序：鼠 牛 虎 兔 龙 蛇 马 羊 猴 鸡 狗 猪
...
def fetch_hk(limit=800):
    """拉取香港六合彩数据（改用 requests 或 urllib，此处保留 urllib）"""
...
def backtest(rows, lookback=200, calib=None):
    ...
    for k in range(total):
        ...
        train_colors = hist_colors[:CONFIG["train_max_len"]][::-1]
        train_normals = hist_normals[:CONFIG["train_max_len"]][::-1]
        train_specials = hist_specials[:CONFIG["train_max_len"]][::-1]

        pred_colors, scores, conf, level, state, mode, can_bet, reason = professional_predict(
            hist_colors, hist_normals, hist_specials, miss_streak, skip_streak, calib
        )
...
def main():
    ...
    # ========== 生成 result.md 报告（用于 GitHub Actions 上传） ==========
